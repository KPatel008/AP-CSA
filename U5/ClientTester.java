    ...
    public static void prefixMerge(Client [] list1, Client [] list2, Client[] result){
        ...
        for (int k = 0; k < result.length; k++) {
            if (list1Counter < list1.length && list2Counter < list2.length) {
                if (list1[list1Counter].compareClient(list2[list2Counter]) < 0) {
                    result[k] = list1[list1Counter];
                    list1Counter++;
                } 
                else if (list1[list1Counter].compareClient(list2[list2Counter]) > 0) {
                    result[k] = list2[list2Counter];
                    list2Counter++;
                } 
                else {
                    result[k] = list1[list1Counter];
            ...
            else if (list1Counter < list1.length) {
                result[k] = list1[list1Counter];
                list1Counter++;
            }
            else {
